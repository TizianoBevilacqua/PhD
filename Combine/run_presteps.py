...
#--------------------------------------------------------------------------------------------------------------------------#
#- USAGE: -----------------------------------------------------------------------------------------------------------------#
#- python3 run_presteps.py --input ../out_dir_syst_090323/ --sig --bkg --data --sig_config config_hdna_input_dir_16cats_diophotonID_newggHBDT_260423_2017.py -#
#--------------------------------------------------------------------------------------------------------------------------#
    ...
    if "fit" not in skip:
        if _cfg["batch"] == "slurm":
            ...
            while not is_job_finished("Syst"):
                ...
                print(f"{num_jobs} syst Jobs are still running...")
                time.sleep(30) 
        ...
        if opt.skip_vtx_split: 
            ...
            skip_vtx = ""
        if opt.groupSignalFitJobsByCat:
            print("grouping fit for categories")
            groups = "--groupSignalFitJobsByCat" 
        else: 
            groups = ""
        os.system(f"python3 RunSignalScripts.py --inputConfig {opt.sconfig} {groups} --mode signalFit --modeOpts \"{skip_vtx} --doPlots {doEffAccFromJson}\"")
    if "package" not in skip:
        if _cfg["batch"] == "slurm":
            # Wait for the job to finish
            while not is_job_finished("signalFit"):
                num_jobs = run_command('squeue -o "%.10i %.9P %.48j %.8u %.8T %.10M %.9l %.6D %R" -u $USER | grep signalFit | wc -l')
                print(f"{num_jobs} Fit jobs are still running...")
                time.sleep(30) 
